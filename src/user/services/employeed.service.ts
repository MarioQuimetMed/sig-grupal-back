...
export class EmployeedService {
  ...
  public async findAllEmployeds(paginationDto: PaginationDto, userId: string): Promise<IPaginatedEmployees<User>> {
    try{
      ...
      console.log(err);
      throw new InternalServerErrorException('Error al buscar los empleados: '+err);
    }
  }

  public async createDistribuitor(createDistribuitorDto :CreateDistribuitorDto): Promise<User> {
    try {
      // Verificar si el email ya existe
      const existingUser = await this.userService.findUserByEmail(createDistribuitorDto.email);
      if (existingUser) {
        throw new NotFoundException(`El usuario con email ${createDistribuitorDto.email} ya existe`);
      }
      ...
      const newDistribuitor = this.userRepostitory.create({
        ...
        password: this.userService.hashPass(`${createDistribuitorDto.name[0]}.${createDistribuitorDto.cellphone}`), // Asignar una contraseña por defecto
      });
      ...
      throw new InternalServerErrorException(`Error al crear el distribuidor: ${err.message}`);
    }

  }

  public async createBulkDistribuitors(bulkDistribuitorDto: BulkDistribuitorDto): Promise<IResponseBulkCreateDist>{
    try{
      const bulkDistribuitor= await converCsvToJson(bulkDistribuitorDto.fileCsv.path);
        ...
        if(!existingUser){
          ...
          await this.userRepostitory.save(
              this.userRepostitory.create({
                  ...
                  email: dist.email,
                  role: UserRole.DISTRIBUTOR,
                  password: this.userService.hashPass(`${dist.name[0]}.${dist.cellphone}`),
              })
          );
          total++;
        }else{
          messages.push(`El distribuidor con email ${dist.email} ya existe y no se creó.`);
        }
      }
      return {
        total,
      ...
      console.log(err);
      throw new InternalServerErrorException(`Error al crear los distribuidores: ${err.message}`);
        ...
        try {
            ...
            Logger.error(`Error deleting temp file: ${error.message}`, 'BulkCreate');
        }
    }
  }


  public async updatedDistribuitor(id: string, updateDistribuitorDto :CreateDistribuitorDto): Promise<User>{
    try {
      ...
      if (updateDistribuitorDto.email !== existingUser.email) {
          const emailExists = await this.userService.findUserByEmail(updateDistribuitorDto.email);
          if (emailExists) {
              throw new BadRequestException(`El email ${updateDistribuitorDto.email} ya está en uso`);
          }
      }
      ...
      Logger.log(`Distribuidor ${id} actualizado exitosamente`, 'UpdateDistribuitor');
      return updatedUser as User;

    } catch (err) {
      Logger.error(`Error actualizando distribuidor: ${err.message}`, 'UpdateDistribuitor');
      
      if (err instanceof NotFoundException || err instanceof BadRequestException) {
          throw err;
      }
      throw new InternalServerErrorException(`Error al actualizar el distribuidor: ${err.message}`);
    }
  }
  ...
  public async deleteDistribuitor(id: string): Promise<User> {
    try {
      ...
      if (!deletedUser) {
        throw new InternalServerErrorException(`Error al eliminar el distribuidor con id ${id}`);
      }

      Logger.log(`Distribuidor ${id} eliminado exitosamente`, 'DeleteDistribuitor');
      return deletedUser as User;

    } catch (err) {
      Logger.error(`Error eliminando distribuidor: ${err.message}`, 'DeleteDistribuitor');
      throw new InternalServerErrorException(`Error al eliminar el distribuidor: ${err.message}`);
    }
  }
  ...
  public async createAdminRunApp(): Promise<void> {
    try{
      ...
      if(!admin){
        ...
        Logger.log(`Se creo el usuario ${createAdmin.name} para el sistema.`, 'InstanceApp');
      }
    }catch (err) {
      throw err;
    }  
